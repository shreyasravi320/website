...
const Grid = (pixels, rows, cols, width, height, bg) => {
    ...
    const pixelSize = Math.min(
        Math.max(
            1,
            height < width
                ? Math.floor((0.85 * height) / rows)
                : Math.floor((0.85 * width) / cols)
        ),
        5
    )
...
const Sim = () => {
    ...
    const processImageUpload = async event => {
        ...
        if (file) {
            ...
            img.onload = async () => {
                ...
                const rgbaBuffer = ctx.getImageData(
                    0,
                    0,
                    canvas.width,
                    canvas.height
                ...
                const rgbBuffer = new Uint8Array(
                    rgbaBuffer.length - rgbaBuffer.length / 4
                )
    ...
    useEffect(() => {
        setHeight(
            window.innerHeight -
                document.querySelector(".pixcell").getBoundingClientRect().top -
                window.scrollY
        )
        setWidth(
            window.innerWidth -
                document.querySelector(".pixcell").getBoundingClientRect()
                    .left -
                window.scrollX
        )
    }, [])
